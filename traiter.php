...
$extensions_autorisees = array( 'mpeg4',  'flv','avi','mp4','3GP','AVI','3gp');
...
if (in_array($extension_upload, $extensions_autorisees) AND in_array($extension_uploadimage, $extensions_autoriseesimage))
{
    try{
        ...
        move_uploaded_file($_FILES['monfichier']['tmp_name'], "00117/".basename($cleanpara21));
        move_uploaded_file($_FILES['imagevignette']['tmp_name'], "00117_vi/". basename($cleanpara22));
        $a=array();
        $b=array();
        $b["chemin"]=$chemin;
    ...
    catch (Exception $ex)
    {
        ...
        echo "impossible d'uploader votre video, probleme du serveur";
    }
...
<h4 class='red-text'>Le type de video  ou le type de la photo est incorrect.</h4><h4 class='green-text'>essayez video mp4 ou 3gp photo vigentte jpg ou png.</h4>";
}
